...
public class AccGeneralRecord extends AccGeneralBaseRecord implements HtmlItemContainer {

  static final HtmlItem[] htmlItems = new HtmlItem[] {
      new HtmlItemString("acc.mailAddress").stringMatchingCondition(PARTIAL, true),
      new HtmlItemString("acc.name").stringMatchingCondition(PARTIAL, true),
      new HtmlItem("acc.role"), new HtmlItemSelect("acc.isValid")};

  public AccGeneralRecord() {
    super();
  }

  public AccGeneralRecord(AccGeneral e, DatetimeFormatParameters params) {
    super(e, params);
  }
